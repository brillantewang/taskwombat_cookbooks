...
directory "#{node[:taskwombat][:path]}" do
  owner 'ubuntu'
  group 'ubuntu'
  mode '0755'
  recursive true
...
# deploy script here
git "#{node[:taskwombat][:path]}" do
  repository node[:taskwombat][:git_repository]
  revision node[:taskwombat][:git_revision]
